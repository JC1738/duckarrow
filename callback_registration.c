...
//
// On Windows (DUCKARROW_NO_CPP_STORAGE), the C++ storage extension is not
// available due to linker limitations, so we provide stub implementations.
//
//===----------------------------------------------------------------------===//
...
void duckarrow_register_go_callbacks(void) {
    // No-op on Windows - C++ storage extension not available
}
